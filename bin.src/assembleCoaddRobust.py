...
from lsst.pipe.tasks.assembleCoadd import AssembleCoaddTask, RobustAssembleCoaddTask
import sys
import argparse
...
if result.help:
    ...
    # As AssembleCoaddTask and RobustAssembleCoaddTask may have different help messages,
    # The appropriate one should be printed
    if result.legacyCoadd:
        AssembleCoaddTask.parseAndRun(args=['--help']+extra[1:])
    else:
        RobustAssembleCoaddTask.parseAndRun(args=['--help']+extra[1:])
...
if result.legacyCoadd:
    ...
    RobustAssembleCoaddTask.parseAndRun(args=extra[1:])
