...
from lsst.pipe.tasks.functors import (
    CompositeFunctor,
    CustomFunctor,
    Column,
    RAColumn,
...
class FunctorTestCase(unittest.TestCase):
        ...
        for ds in self.datasets:
            for filterName in self.filters:
                ...
                df.columns = pd.MultiIndex.from_tuples(
                    [(ds, filterName, c) for c in df.columns], names=("dataset", "filter", "column")
                )
    ...
    def testColumn(self):
        ...
        self.dataDict["base_FootprintArea_value"] = np.full(self.nRecords, 1)
    ...
    def testCustom(self):
        self.columns.append("base_FootprintArea_value")
        self.dataDict["base_FootprintArea_value"] = np.random.rand(self.nRecords)
        parq = self.simulateMultiParquet(self.dataDict)
    ...
    def testCoords(self):
        ...
        columnDict = {"dataset": "ref", "filter": "HSC-G", "column": ["coord_ra", "coord_dec"]}
        coords = parq.toDataFrame(columns=columnDict, droplevels=True) / np.pi * 180.0
    ...
    def testMag(self):
        ...
        for dataset in ["forced_src", "meas"]:
            psfMag_G = self._funcVal(Mag(fluxName, dataset=dataset, filt="HSC-G"), parq)
            psfMag_R = self._funcVal(Mag(fluxName, dataset=dataset, filt="HSC-R"), parq)

            psfColor_GR = self._funcVal(Color(fluxName, "HSC-G", "HSC-R", dataset=dataset), parq)
    ...
    def testComposite(self):
        ...
        funcDict = {
            ...
            "cmodel_magDiff": MagDiff("base_PsfFlux", "modelfit_CModel", filt=filt),
        }
    ...
    def testLocalPhotometry(self):
        ...
        self.dataDict["base_PsfFlux_instFluxErr"] = np.full(self.nRecords, fluxErr)
        self.dataDict["base_LocalPhotoCalib"] = np.full(self.nRecords, calib)
        self.dataDict["base_LocalPhotoCalibErr"] = np.full(self.nRecords, calibErr)
        parq = self.simulateMultiParquet(self.dataDict)
        func = LocalPhotometry(
            "base_PsfFlux_instFlux",
            "base_PsfFlux_instFluxErr",
        ...
        nanoJansky = func.instFluxToNanojansky(
            df[("meas", "HSC-G", "base_PsfFlux_instFlux")], df[("meas", "HSC-G", "base_LocalPhotoCalib")]
        )
        ...
        self.assertTrue(np.allclose(nanoJansky.values, flux * calib, atol=1e-13, rtol=0))
        self.assertTrue(
            np.allclose(mag.values, (flux * calib * u.nJy).to_value(u.ABmag), atol=1e-13, rtol=0)
        )
        self.assertTrue(
            np.allclose(nanoJanskyErr.values, np.hypot(fluxErr * calib, flux * calibErr), atol=1e-13, rtol=0)
        )
        self.assertTrue(
            np.allclose(
                magErr.values,
        ...
        self._testLocalPhotometryFunctors(LocalNanojansky, parq, nanoJansky)
        self._testLocalPhotometryFunctors(LocalNanojanskyErr, parq, nanoJanskyErr)
        self._testLocalPhotometryFunctors(LocalMagnitude, parq, mag)
        self._testLocalPhotometryFunctors(LocalMagnitudeErr, parq, magErr)
    ...
    def _testLocalPhotometryFunctors(self, functor, parq, testValues):
        ...
        val = self._funcVal(func, parq)
        self.assertTrue(np.allclose(testValues.values, val.values, atol=1e-13, rtol=0))
    ...
    def testConvertPixelToArcseconds(self):
        ...
        localWcsPlugin = measBase.EvaluateLocalWcsPlugin(
            None, "base_LocalWcs", afwTable.SourceTable.makeMinimalSchema(), None
        )
        for dec in np.linspace(-90, 90, 10):
            for x, y in zip(
                ...
                linAffMatrix = localWcsPlugin.makeLocalTransformMatrix(wcs, center)
                ...
                self.dataDict["base_LocalWcs_CDMatrix_1_1"] = np.full(self.nRecords, linAffMatrix[0, 0])
                self.dataDict["base_LocalWcs_CDMatrix_1_2"] = np.full(self.nRecords, linAffMatrix[0, 1])
                self.dataDict["base_LocalWcs_CDMatrix_2_1"] = np.full(self.nRecords, linAffMatrix[1, 0])
                self.dataDict["base_LocalWcs_CDMatrix_2_2"] = np.full(self.nRecords, linAffMatrix[1, 1])
                parq = self.simulateMultiParquet(self.dataDict)
                func = LocalWcs(
                ...
                for centX, centY, sep in zip(
                    testPixelDeltas[:, 0], testPixelDeltas[:, 1], sepRadians.values
                ):
                    afwSepRadians = skyOrigin.separation(wcs.pixelToSky(x + centX, y + centY)).asRadians()
                    self.assertAlmostEqual(1 - sep / afwSepRadians, 0, places=6)

                # Test the pixel scale computation.
                func = ComputePixelScale(
                    "base_LocalWcs_CDMatrix_1_1",
                    "base_LocalWcs_CDMatrix_1_2",
                    "base_LocalWcs_CDMatrix_2_1",
                    "base_LocalWcs_CDMatrix_2_2",
                ...
                self.assertTrue(
                    np.allclose(
                        wcs.getPixelScale(center).asArcseconds(), pixelScale.values, rtol=1e-8, atol=0
                    )
                )

                func = ConvertPixelToArcseconds(
                    "dipoleSep",
                    "base_LocalWcs_CDMatrix_1_1",
                    "base_LocalWcs_CDMatrix_1_2",
                    "base_LocalWcs_CDMatrix_2_1",
                ...
                val = self._funcVal(func, parq)
                self.assertTrue(
                    np.allclose(pixelScale.values * dipoleSep, val.values, atol=1e-16, rtol=1e-16)
                )
