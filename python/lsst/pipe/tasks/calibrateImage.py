...
class CalibrateImageConfig(pipeBase.PipelineTaskConfig, pipelineConnections=CalibrateImageConnections):
    ...
    do_dynamic_detection = pexConfig.Field(
        dtype=bool,
        default=True,
        doc="Implement the dynamic detection approach?",
    )
    ...
    def validate(self):
        ...
        if not self.do_dynamic_detection:
            if not self.psf_detection.reEstimateBackground:
                raise pexConfig.FieldValidationError(
                    CalibrateImageConfig.psf_detection,
                    self,
                    "If not using the dynamic detection implementation (i.e. "
                    "config.do_dynamic_detection = False), CalibrateImageTask.psf_detection "
            ...
            if not self.star_detection.reEstimateBackground:
                raise pexConfig.FieldValidationError(
                    ...
                    self,
                    "If not using the dynamic detection implementation (i.e. "
                    "config.do_dynamic_detection = False), CalibrateImageTask.star_detection "
                    "background must be configured with "
                    "reEstimateBackground = True to maintain original behavior."
                )
...
class CalibrateImageTask(pipeBase.PipelineTask):
    ...
    def run(
        ...
        *,
        ...
        try:
            ...
            if self.config.do_dynamic_detection:
                self._remeasure_star_background(result)
    ...
    def _compute_psf(self, exposure, id_generator, background_to_photometric_ratio=None):
        ...
        if not self.config.do_dynamic_detection:
    ...
    def _remeasure_star_background(self, result):
        """Remeasure the exposure's background with iterative detection.
        ...
        # using a dynamic detection iteration to ensure a "Goldilocks
        # Zone" for the fraction of detected pixels.
        ...
        while inBackgroundDet:
            ...
            self.log.info("nIter = %d, thresh = %.2f: Fraction of pixels marked as DETECTED or "
                          "DETECTED_NEGATIVE in star_background_detection = %.3f (max is %.3f)",
