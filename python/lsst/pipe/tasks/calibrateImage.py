...
class CalibrateImageTask(pipeBase.PipelineTask):
    ...
    def _compute_psf(self, result, id_generator, ref_cat_source_density=None):
        ...
        if not self.config.do_adaptive_threshold_detection:
            ...
            adaptiveRepairMinDnFactor = 1.0
            if ref_cat_source_density > 5000.0:
                adaptiveRepairMinDnFactor = 2.0e-6*ref_cat_source_density + 1.0
                adaptiveRepairMinDn *= adaptiveRepairMinDnFactor
                adaptivePsfRepairConfig.cosmicray.min_DN = adaptiveRepairMinDn
            self.log.debug("ref_cat_source_density: %.1f; adaptiveRepairMinDn: %.2f; "
                           "adaptiveRepairMinDnFactor: %.2f",
                           ref_cat_source_density, adaptiveRepairMinDn, adaptiveRepairMinDnFactor)
            ...
            for ia, amp in enumerate(amps):
                ...
                if detected_fraction_amp < 0.002:
                    ...
                    if n_no_zero_det_amps > 2:
                        ...
                        break
                highest_detected_fraction_per_amp = np.nanmax([detected_fraction_amp,
    ...
    def _remeasure_star_background(self, result, background_to_photometric_ratio=None):
        ...
        for nIter in range(maxIter):
            ...
            if nFootprintTemp == 0:
                ...
                # The following is to accommodate a relatively hopeless fully
                # saturated field (e.g center of 47Tuc).
                if starBackgroundDetectionConfig.thresholdValue > 1500:
                    minFootprints = 2
                if starBackgroundDetectionConfig.thresholdValue > 2000:
                    minFootprints = 1
                nZeroEncountered += 1
            ...
            if detected_fraction > maxDetFracForFinalBg or nFootprintTemp <= minFootprints:
                newThreshTemp = 1.1*currentThresh
                if detected_fraction > maxDetFracForFinalBg:
                    newThreshTemp *= 1.15
                if detected_fraction > 0.98:
                    newThreshTemp *= 1.1
                if nFootprintTemp <= minFootprints:
                    newThreshTemp *= 1.15
                if nFootprintTemp <= 1:
