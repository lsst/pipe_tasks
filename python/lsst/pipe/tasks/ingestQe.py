...
from .read_qe_curves import read_all_curves
...
class IngestQeCurvesArgumentParser(InputOnlyArgumentParser):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.add_argument("root", help="Root directory to scan for qe curves.")


class IngestQeCurvesTask(IngestCalibsTask):
    """Task that generates registry for calibration images"""
    ArgumentParser = IngestQeCurvesArgumentParser
    _DefaultName = "ingestQeCurves"
    ...
    def run(self, args):
        ...
        try:
            ...
            curves = read_all_curves(args.root, camera)
            file_names = []
            for d in curves:
                for s in curves[d]:
                    file_name = f'curve_{d}_{s.isoformat()}.fits'
                    full_file_name = os.path.join(temp_dir, file_name)
                    self.log.info('Curve written for sensor: %s and calibDate: %s' %
                                  (d, s.isoformat()))
                    curves[d][s].writeFits(full_file_name)
