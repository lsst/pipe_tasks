...
class ProcessBrightStarsConnections(PipelineTaskConnections, dimensions=("instrument", "visit", "detector")):
    ...
    def __init__(self, *, config=None):
        config = cast(ProcessBrightStarsConfig, config)
...
class ProcessBrightStarsConfig(PipelineTaskConfig, pipelineConnections=ProcessBrightStarsConnections):
    ...
    magLimit = Field[float](
        doc="Magnitude limit, in Gaia g-band; all stars brighter than this value will be processed.",
        default=18,
    )
...
class ProcessBrightStarsTask(PipelineTask):
    ...
    def __init__(self, butler=None, initInputs=None, *args, **kwargs):
        ...
        self.setModelStamp()
        # Configure the reference catalog.
        if butler is not None:
            self.makeSubtask("refObjLoader", butler=butler)

    def runQuantum(self, butlerQC, inputRefs, outputRefs):
        self.config = cast(ProcessBrightStarsConfig, self.config)
    ...
    def run(self, inputExposure, refObjLoader=None, dataId=None, skyCorr=None):
        ...
        statsControl = StatisticsControl(numSigmaClip=self.config.numSigmaClip, numIter=self.config.numIter)
    ...
    def _replaceSecondaryFootprints(self, stamp, coordPix, objectId, find="DETECTED", replace="BAD"):
        ...
        if (numPrimaryFootprints := len(allFootprints) - len(secondaryFootprints)) != 1:
            self.log.warning(
