        ...
        for tempExpInd, calExpSubsetRefList in enumerate(tempExpIdDict.itervalues()):
            ...
            for calExpInd, calExpRef in enumerate(calExpSubsetRefList):
                ...
                try:
                    ...
                    if numGoodPix != 0 or self.config.provenance.saveEmptyCcds:
                        self.provenance.addCcdToTempExp(coaddInputs, tempExpInd, numGoodPix,
                                                        exposure, calExpRef)

                except Exception, e:
                    self.log.warn("Error processing calexp %s; skipping it: %s" % \
                        (calExpRef.dataId, e))
                    if self.config.provenance.saveErrorCcds:
                        self.provenance.addCcdToTempExp(coaddInputs, tempExpInd, numGoodPix,
                                                        None, calExpRef)
                    continue
            ...
            self.provenance.makeCoaddTempExp(coaddInputs, tempExpInd, totGoodPix, coaddTempExp)
