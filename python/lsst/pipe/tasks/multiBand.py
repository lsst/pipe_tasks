...
class MeasureMergedCoaddSourcesTask(PipelineTask, CmdLineTask):
    ...
    def runQuantum(self, butlerQC, inputRefs, outputRefs):
        ...
        table = afwTable.SourceTable.make(self.schema, idFactory)
        sources = afwTable.SourceCatalog(table)
        sources.extend(inputs.pop('intakeCatalog'), self.schemaMapper)
        ...
        tractNumber = inputRefs.intakeCatalog.dataId['tract']
        tractInfo = skyMap[tractNumber]
        patchInfo = tractInfo.getPatchInfo(inputRefs.intakeCatalog.dataId['patch'])
        skyInfo = Struct(
            skyMap=skyMap,
            tractInfo=tractInfo,
            patchInfo=patchInfo,
