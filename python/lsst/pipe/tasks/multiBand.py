    ...
    **kwargs
    ...
    def runQuantum(self, butlerQC, inputRefs, outputRefs):
        ...
        try:
            ...
            if self.config.writeEmptyBackgrounds:
                butlerQC.put(self.makeEmptyBackground(exposure, patchInfo), outputRefs.outputBackgrounds)
    ...
    def run(self, exposure, idFactory, expId, patchInfo=None):
        ...
        if self.config.forceExactBinning:
            exposure = self._cropExposure(exposure, patchInfo)
        if self.config.doScaleVariance:
    ...
    def _makeEmptyBackground(self, exposure, patchInfo=None):
        ...
        if self.config.forceExactBinning:
            exposure = self._cropExposure(exposure, patchInfo).clone()
