...
class SubtractBrightStarsTask(PipelineTask):
    ...
    def run(
        ...
        # The number of 90 degree rotations required to revers each stamp's
        # rotation
    ...
    def scaleModel(self, model, star, inPlace=True, nb90Rots=0, psf_annular_flux=1.0):
        ...
        psf_annular_flux: `float`, optional
            The annular flux of the PSF model at the radius where the flux of
            the given star is determined. This is 1 for stars present in
            inputBrightStarStamps, but can be different for stars that are
    ...
    def _overrideWarperConfig(self):
        ...
        This override is necessary for stars that are missing from the
        inputBrightStarStamps object but still need to be subtracted.
        """
    ...
    def setMissedStarsStatsControl(self):
        ...
        self.missedStatsControl = StatisticsControl(
            numSigmaClip=self.warper.config.numSigmaClip, numIter=self.warper.config.numIter
        )
        self.missedStatsFlag = stringToStatisticsProperty(self.warper.config.annularFluxStatistic)

    def setWarpTask(self):
        """Create an instance of ProcessBrightStarsTask that will be used to
        produce stamps of stars to be subtracted.
        """
    ...
    def makeBrightStarList(self, inputBrightStarStamps, inputExposure, refObjLoader):
        ...
        inputBrightStarStamps to be subtracted.
        ...
        brightStarList:
            A list containing
            `lsst.meas.algorithms.brightStarStamps.BrightStarStamp` of stars to
            be subtracted.
        """
        self.setWarpTask()
    ...
    def createAnnulus(self, brightStarStamp):
        """Create a circular annulus around the given star.

        The circular annulus is set based on the inner and outer optimal radii.
        These radii describe the annulus from where the flux of the star is found.
        The aim is to create the same annulus for the PSF model, eventually
        measuring the model flux around that annulus.
        An optimal radius usually differs from the radius where the PSF model
        is normalized.
    ...
    def findPsfAnnularFlux(self, brightStarStamp, maskedModel):
        """Find the annular flux of the PSF model within a determined annulus.

        This flux will be used for re-scaling the PSF to the level of stars
        with bad stamps. Stars with bad stamps are those without a flux within
        the normalization annulus.
    ...
    def findPsfAnnularFluxes(self, brightStarStamps):
        ...
        PsfAnnularFluxes: numpy.array
            A two column numpy.array containing annular fluxes of the PSF at
            radii where the flux for stars exist (could be found).

        Notes
        -----
        While the PSF model is normalized at a certain radius, the annular flux
        of a star around that radius might be impossible to find. Therefore, we
        have to scale the PSF model considering a radius where the star has an
        identified flux. To do that, the flux of the model should be found and
        used to adjust the scaling step.
        """
        outerRadii = []
        annularFluxes = []
    ...
    def preparePlaneModelStamp(self, brightStarStamp):
        """Prepare the PSF plane model stamp.

        It is called PlaneMdoel because while it is a PSF model stamp that is
        warped and rotated to the same orientation of a chosen star, but it is
        not scaled to the brightness level of the star yet.
        ...
        bbox: `~lsst.geom.Box2I`
            Contains the corner coordination and the dimensions of the model
            stamp.

        invImage: `~lsst.afw.image.MaskedImageF`
            The extended PSF model, shifted (and potentially warped and
            rotated) to match the bright star position.
        ...
        Notes
        -----
        Since detectors have different orientations, the PSF model should be
        rotated to match the orientation of the detectors in some cases. To do
        that, the code uses the inverse of the transform that is applied to the
        bright star stamp to match the orientation of the detector.
        """
        # Set the origin.
    ...
    def addScaledModel(self, subtractor, brightStarStamp, multipleAnnuli=False):
        ...
        subtractor : `~lsst.afw.image.MaskedImageF`
            The full image containing the scaled model of bright stars to be
            subtracted from the input exposure.
        brightStarStamp :
            `~lsst.meas.algorithms.brightStarStamps.BrightStarStamp`
            The stamp of the star of which the PSF model will be scaled and
            added to the subtractor.
        multipleAnnuli : bool, optional
            If true, the model should be scaled based on a flux at a radius
            other than its normalization radius.

        Returns
        -------
        subtractor : `~lsst.afw.image.MaskedImageF`
            The input subtractor full image with the added scaled model at the
            given star's location in the exposure.
        invImage: `~lsst.afw.image.MaskedImageF`
            The extended PSF model, shifted (and potentially warped) to match
            the bright star position.
        """
    ...
    def buildSubtractor(self, brightStarStamps, subtractor, invImages, multipleAnnuli=False):
        """Build an image containing potentially multiple scaled PSF models,
        each at the location of a given bright star.
        ...
        brightStarStamps :
            ...
            `~lsst.pipe.tasks.processBrightStars.ProcessBrightStarsTask`.
        subtractor : `~lsst.afw.image.MaskedImageF`
            The Exposure that will contain the scaled model of bright stars to
            be subtracted from the exposure.
        invImages : `list`
            A list containing extended PSF models, shifted (and potentially
            warped) to match the bright stars positions.
        multipleAnnuli : bool, optional
        ...
        subtractor : `~lsst.afw.image.MaskedImageF`
            ...
            input exposure.
        invImages: list
            A list containing the extended PSF models, shifted (and potentially
