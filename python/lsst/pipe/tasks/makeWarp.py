...
class MakeWarpTask(CoaddBaseTask):
    ...
    def _prepareCalibratedExposures(self, *, visitSummary, calExpList=[], backgroundList=None,
                                    skyCorrList=None, **kwargs):
        ...
        for index, (calexp, background, skyCorr) in enumerate(zip(calExpList,
            ...
            if (photoCalib := row.getPhotoCalib()) is not None:
                ...
                self.log.warning(
                    ...
                    "not be used in the warp", calexp.dataId["detector"], calexp.dataId["visit"],
                )
                continue
