...
class SolarSystemAssociationTask(pipeBase.Task):
    ...
    def run(self, diaSourceCatalog, ssObjects, visitInfo, bbox, wcs):
        ...
        for ssObject in maskedObjects:
            ...
            ssoVect = self._radec_to_xyz(ssObject["ephRa"], ssObject["ephDec"])
            ...
            if len(idx) == 1 and np.isfinite(dist[0]):
                ...
                all_cols = ["ObjID", "phaseAngle", "helioRange",
                ...
                residual_ras.append(dia_ra - ssObject["ephRa"])
                residual_decs.append(dia_dec - ssObject["ephDec"])
                prov_ids.append(ssObject['ObjID'])
    ...
    def _return_empty(self, diaSourceCatalog, emptySolarSystemObjects):
        ...
        dia_columns = [col for col in ssp.ssobject.DIA_COLUMNS if not col == 'diaSourceId']
